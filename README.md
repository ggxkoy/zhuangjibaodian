...
> `review/no-ai-personal` 是个人主体审核分支：微信小程序仅保留结构化规则推荐、价格查询与固定资料展示，不包含自由问答或模型调用。验收说明见 [docs/NO-AI-REVIEW.md](docs/NO-AI-REVIEW.md)。
...
- **老板娘 galgame 界面**：小程序首页是视觉小说式界面——全屏立绘（`miniprogram/assets/boss.png`，缺图自动降级）+ 底部对话框打字机 + 点击推进 + 居中选项肢 + 回想记录 + 表情差分（说话/开心/沉思随剧情切换）；"12000 玩3A"一句话出方案（右上角方案面板，点开进明细），追问、换一套都在剧情流里。本地意图解析兜底（预算+用途识别、型号数字防误判），没有 AI 也能对话出方案。H5 端为右下角"👩‍💼 问老板娘"悬浮入口
- **变现（预置，配置驱动）**：CPS 返佣——配好联盟推广位（`JD_UNION_ID`/`PDD_PID`）后比价链接自动转带佣推广链接，转链失败回退搜索链接；流量主广告——`miniprogram/utils/ads.js` 填入 adUnitId 后生效（明细页 banner、离页插屏、激励视频解锁角色皮肤/额外换一套），全部未配置时无任何广告与门槛。操作手册见 [docs/MONETIZE.md](docs/MONETIZE.md)
- **自定义老板娘**：👗 面板可切换预设店主（姐系老板娘/元气学妹小雫/毒舌御姐绫姐，人设+开场白+立绘整套切换）或完全自定义（起名+写性格+上传立绘），本地持久化。人设只影响口吻——**报价纪律是铁律**，服务端/云函数强制附加，自定义人设写"价格随便编"也改不掉。美术素材生成清单见 [docs/ART-ASSETS.md](docs/ART-ASSETS.md)
