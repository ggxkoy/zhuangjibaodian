...
- **老板娘多轮对话**：小程序**首页即对话**——"12000 玩3A"一句话出配置单卡片，追问、换一套都在聊天流里；本地意图解析兜底（预算+用途识别、型号数字防误判），没有 AI 也能对话出方案。人设铁律：只聊装机、价格只引用系统比价数据绝不编造、拿不准直说；对话自动携带当前方案与各件价位判定作为上下文。H5 端为右下角"👩‍💼 问老板娘"悬浮入口
