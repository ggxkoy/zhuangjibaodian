...
function lowestMatched(items, part) {
  ...
  for (const it of items) {
    ...
    if (!best || it.price < best) best = it.price;
  }
...
const adapters = {
  ...
  async jd(part) {
    ...
    params.sign = md5(secret + base + secret).toUpperCase();
    ...
    return lowestMatched(items, part);
  },
  ...
  async taobao(part) {
    ...
    const items = list.map(g => ({ title: g.title, price: parseFloat(g.zk_final_price) }));
    return lowestMatched(items, part);
  },
