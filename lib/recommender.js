...
const USAGE_PROFILES = {
  ...
  productivity: {
    ...
    minBudget: 3200,
    minRamGB: b => (b >= 9000 ? 32 : 16) // 生产力内存下限：行情再贵也不能配16G跑渲染/剪辑
  },
  office: {
    label: '办公',
...
function pickCase(board) {
  return PARTS.case.filter(c => c.fits.includes(board.formFactor))
    .sort((a, b) => a.refPrice - b.refPrice)[0];
}
...
function assemble(cpu, gpu, budget, profile) {
  ...
  let ram = pickRam(board, Math.max(budget * profile.ramTarget, 249));
  const minRam = profile.minRamGB ? profile.minRamGB(budget) : 0;
  if (ram.capacity < minRam) {
    const ok = PARTS.ram.filter(r => r.type === board.ramType && r.capacity >= minRam)
      .sort((a, b) => a.refPrice - b.refPrice)[0];
    if (!ok) return null;
    ram = ok;
  }
  ...
  if (budget >= 10000) {
    upgrades.push(['case', PARTS.case.filter(c => c.fits.includes(board.formFactor)), (a, b) => a.refPrice > b.refPrice]);
  }
  if (budget >= 12000) {
...
function recommend(budget, usage, exclude = []) {
  ...
  for (const cpu of cpus) {
    for (const gpu of gpus) {
      ...
      if (gpu && cpu.game < gpu.game * 0.6) continue; // CPU太弱会瓶颈高端显卡
