    ...
    const req = https.request({
      ...
      res.on('end', () => {
        try {
          ...
          if (!String(content || '').trim()) return reject(new Error('DeepSeek 未返回内容'));
          resolve(content);
...
function parseJsonContent(content, label) {
  ...
  if (start < 0 || end <= start) throw new Error(`${label}返回的 JSON 不完整`);
  try {
...
async function chatJson(messages, { temperature = 0.2, maxTokens = 400, label = 'DeepSeek' } = {}) {
  const limits = [maxTokens, Math.max(maxTokens * 2, 600)];
  let lastError;
  for (const limit of limits) {
    const content = await chat(messages, { json: true, temperature, maxTokens: limit });
    try {
...
async function parseRequirement(text) {
  ...
  ], { temperature: 0.1, maxTokens: 200, label: '需求解析' });
...
async function elicit(history, character) {
  const r = await chatJson([
    ...
    ...history.slice(-10)
  ], { temperature: 0.4, maxTokens: 300, label: '对话需求' });
...
async function digestReviews(partName, comments) {
  ...
  ], { temperature: 0.2, maxTokens: 500, label: '口碑提炼' });
}
