...
小程序**首页即老板娘对话**（chat-first）：需求在对话里说，配置单以聊天卡片形式出现，
点卡片进明细页；传统表单页保留为次级入口（开场快捷选项里的「📋 表单模式」）。

- [ ] 首页是对话界面：老板娘欢迎语 + 一排快捷选项（6000 玩网游 / 12000 玩3A…）
- [ ] 发送「12000 玩3A」→ 老板娘回复过渡语 → 消息流里出现**配置单卡片**（用途标签/合计/CPU/显卡/内存三行）
      —— 此路径走**本地意图解析**，不依赖 AI，无 key 也必须可用
- [ ] 发送「8000」（只有预算）→ 老板娘追问用途 + 选项；点「玩游戏」→ 出方案卡
- [ ] 发送「5070Ti怎么样」→ **不应**误出方案（型号数字防误判；无 AI 时回引导话术，有 AI 时老板娘作答）
- [ ] 发送「换一套」→ 新配置卡片，核心件与上一套不同
- [ ] 点配置卡片 → 进入明细页（价格徽章/走势/口碑/经验齐全）；明细页「问老板娘」→ 返回对话
- [ ] 快捷选项「📋 表单模式」→ 进入传统表单页，功能与旧版一致
- [ ] 表单页：输入 `8000` → 选「游戏」→ 生成 → 跳转方案页，合计不超预算
